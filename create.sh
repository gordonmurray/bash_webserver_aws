...
aws ec2 create-security-group --region ${REGION} --group-name ${SECURITY_GROUP_NAME} --vpc-id ${VPC_ID} --description "For an example EC2 instance"
...
aws ec2 authorize-security-group-ingress --region ${REGION} --group-id ${SECURITY_GROUP_ID} --to-port 80 --ip-protocol tcp --cidr-ip 0.0.0.0/0 --from-port 80

# Add a rule to the security group to allow port 22 open to this machine
aws ec2 authorize-security-group-ingress --region ${REGION} --group-id ${SECURITY_GROUP_ID} --to-port 22 --ip-protocol tcp --cidr-ip ${MY_IP_ADDRESS}/32 --from-port 22
